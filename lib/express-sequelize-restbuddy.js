 ...
 *
 * @readonly
 * @type {{show: show, list: list, update: update, replace: replace, create: create, destroy: destroy}}
 */
var requestHandlers = Object.freeze({
  ...
  replace: function replace (model, queryOptions, options, req, res, next) {
    model.findOne(queryOptions)
      .then(function (item) {
        if (!item) {
          return res.status(404).json({ message: model.name + ' not found' });
        ...
        // Create a new model instance with the default values, in order to
        // reset all fields that are not specified to their default values.
        var defaultValues = model.build({ id: item.id }).dataValues;
        return item.updateAttributes(_.merge(defaultValues, req.body))
          .then(function (item) {
            req.data = item;
            next();
          });
...
function requestType (req) {
  ...
  } else if (req.method === 'PATCH' && isParam(_.last(pathComponents))) {
    return RequestTypes.Update;
  } else if (req.method === 'PUT' && isParam(_.last(pathComponents))) {
